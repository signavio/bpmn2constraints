...
def test_element_is_marked_as_end_if_it_has_no_successors():
    ...
    for elem in res:
        if elem.get("is end"):
            assert elem.get("id") == LINEAR_SEQUENCE_DIAGRAM_WITHOUT_START_AND_END.get("end element id")
...
def test_element_is_marked_as_end_if_successor_is_end_event():
    ...
    for elem in res:
        if elem.get("is end"):
            assert elem.get("id") == LINEAR_SEQUENCE_DIAGRAM_WITH_START_AND_END.get("end element id")

def test_all_start_elements_are_marked_as_ending_if_multiple_endings():
    res = init_test_setup(MULTIPLE_ENDINGS_DIAGRAM)
    end_count = 0
    for elem in res:
        if elem.get("is end") and elem.get("id") in MULTIPLE_ENDINGS_DIAGRAM.get("ending elements"):
            end_count += 1
