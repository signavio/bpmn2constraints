...
from test_utils import init_test_setupt_for_mermaid


def test_linear_mermaid_graph():
    res = init_test_setupt_for_mermaid(LINEAR_MERMAID_GRAPH)
    assert LINEAR_MERMAID_GRAPH.get("output") == res


def test_gateway_mermaid_graph():
    res = init_test_setupt_for_mermaid(GATEWAY_MERMAID_GRAPH)
