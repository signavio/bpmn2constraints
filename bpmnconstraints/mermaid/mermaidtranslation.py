...
class Mermaid:
    ...
    def __match_successor_str(self, successor):
        ...
        if successor["is end"]:
            return self.__gen_event_str(successor_id, successor["name"])
        elif successor["type"] in ALLOWED_ACTIVITIES:
            return self.__gen_activity_str(successor_id, successor["name"])
        elif successor["type"] in ALLOWED_GATEWAYS:
            return self.__gen_gateway_str(successor_id, successor["name"])

    def __create_node(self, elem, gen_str_func):
        rows = []
        for successor in elem["successor"]:
            node_id = self.__gen_new_id(elem["id"])
            row = f"{gen_str_func(node_id, elem['name'])}{SEQUENCE_FLOW}{self.__match_successor_str(successor)}"
            rows.append(row)
        return rows
