...
def to_ltl(constraint_str):
    ...
    if templ_str == ABSENCE:
        ...
        if n == 1:
            return And(to_ltl(constraint_str.replace(EXACTLY, EXISTENCE)),
                       to_ltl(constraint_str.replace(EXACTLY + "1", ABSENCE + "2")))
        elif n == 2:
            return And(to_ltl(constraint_str.replace(EXACTLY, EXISTENCE)),
                       to_ltl(constraint_str.replace(EXACTLY + "2", ABSENCE + "3")))
        elif n == 3:
            return And(to_ltl(constraint_str.replace(EXACTLY, EXISTENCE)),
                       to_ltl(constraint_str.replace(EXACTLY + "3", ABSENCE + "4")))
        else:
            raise ValueError("Unsupported n: " + str(n))
        ...
        return And(to_ltl(constraint_str.replace(SUCCESSION, RESPONSE)),
                   to_ltl(constraint_str.replace(SUCCESSION, PRECEDENCE)))

    elif templ_str == ALTERNATE_SUCCESSION:
        return And(to_ltl(
            constraint_str.replace(ALTERNATE_SUCCESSION, ALTERNATE_RESPONSE)),
            to_ltl(constraint_str.replace(ALTERNATE_SUCCESSION,
                                          ALTERNATE_PRECEDENCE)))

    elif templ_str == CHAIN_SUCCESSION:
        return And(
            to_ltl(constraint_str.replace(CHAIN_SUCCESSION, CHAIN_RESPONSE)),
            to_ltl(constraint_str.replace(CHAIN_SUCCESSION, CHAIN_PRECEDENCE)))
        ...
        return And(to_ltl(constraint_str.replace(NOT_SUCCESSION, NOT_RESPONSE)),
                   to_ltl(constraint_str.replace(NOT_SUCCESSION, NOT_PRECEDENCE)))
    else:
